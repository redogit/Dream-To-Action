...
CSS = r'''
:root { color-scheme: light; --ink:#152d35; --muted:#425a61; --paper:#f5f4ef; --white:#fff; --line:#b8c6c8; --accent:#07584f; --soft:#e9f1ed; --warn:#713b00; --error:#9f1d20; }
* { box-sizing:border-box; }
body { margin:0; background:var(--paper); color:var(--ink); font:1.05rem/1.6 system-ui,-apple-system,"Segoe UI",sans-serif; }
...
.skip { position:absolute; top:-8rem; left:1rem; background:white; padding:1rem; z-index:10; }
.skip:focus { top:1rem; }
header,main,footer { max-width:72rem; margin:auto; padding:1.5rem; }
header { padding-top:2.5rem; padding-bottom:1rem; }
.kicker { text-transform:uppercase; letter-spacing:.13em; font-size:.8rem; font-weight:750; }
h1 { font-size:clamp(2.3rem,5vw,3.8rem); line-height:1.12; margin:.5rem 0 1rem; letter-spacing:-.035em; }
h2 { line-height:1.25; font-size:1.55rem; margin:0 0 1rem; }
h3 { font-size:1.12rem; margin:1.5rem 0 .4rem; }
p { max-width:72ch; }
.lead { font-size:1.2rem; max-width:58ch; }
.muted,.hint { color:var(--muted); }
.hint { font-size:.94rem; margin:.35rem 0 .7rem; }
.notice { border-left:5px solid var(--accent); background:var(--soft); padding:1rem 1.3rem; margin:1.1rem 0; }
.notice p { margin:.2rem 0; }
.layout { display:grid; grid-template-columns:minmax(0,1.2fr) minmax(0,1fr); gap:1.5rem; align-items:start; }
.panel { background:var(--white); border:1px solid var(--line); border-radius:.8rem; padding:1.5rem; margin-bottom:1.5rem; min-width:0; }
fieldset { border:0; margin:0 0 1.5rem; padding:0; min-width:0; }
legend { font-size:1.15rem; font-weight:750; margin-bottom:1rem; padding:0; }
.field { margin-bottom:1rem; }
label { display:block; font-weight:650; margin-bottom:.4rem; }
input,textarea,select { display:block; width:100%; border:1.5px solid #758d94; border-radius:.35rem; padding:.65rem; background:white; color:var(--ink); min-height:2.9rem; }
textarea { min-height:6.5rem; resize:vertical; }
[aria-invalid="true"] { border:2px solid var(--error); }
.buttons { display:flex; gap:.75rem; flex-wrap:wrap; margin:1rem 0; }
button { min-height:2.85rem; padding:.65rem 1rem; border:2px solid var(--accent); border-radius:.4rem; background:var(--accent); color:white; cursor:pointer; font-weight:650; }
button.secondary { color:var(--accent); background:white; }
button.danger { border-color:var(--error); color:var(--error); background:white; }
button:hover { text-decoration:underline; }
button:disabled { opacity:.6; cursor:not-allowed; }
small { font-size:.9rem; }
summary { font-weight:650; cursor:pointer; padding:.5rem 0; }
details { margin:.75rem 0; }
.error { border:2px solid var(--error); padding:1rem; margin-bottom:1rem; }
.error p { margin-top:0; font-weight:700; }
#status { min-height:1.6em; font-weight:650; }
.pill { display:inline-block; padding:.25rem .65rem; background:var(--soft); border:1px solid var(--accent); border-radius:.3rem; font-weight:650; }
...
#history-list li { margin-bottom:1.3rem; padding-left:.2rem; }
#history-list p { margin:.3rem 0; white-space:pre-wrap; overflow-wrap:anywhere; }
.meta { font-size:.9rem; color:var(--muted); }
.rule { font-size:1.15rem; font-weight:650; }
footer { padding-bottom:3rem; border-top:1px solid var(--line); }
[hidden] { display:none!important; }
@media(max-width:50rem) { .layout { grid-template-columns:1fr; } header,main,footer { padding:1.1rem; } .panel { padding:1.1rem; } }
@media print { body { background:white; color:black; font-size:11pt; } header,footer { padding:.4rem; } main { padding:0; } .layout { display:block; } .no-print,#status,#errors,#edit-panel,#review-panel,#file-panel { display:none!important; } .panel { border:0; padding:0; } h1 { font-size:24pt; } #preview-panel { display:block; } #history-list li { break-inside:avoid; } }
'''
HTML = r'''<!doctype html>
<html lang="en">
<head>
...
<meta name="description" content="A local, person-directed workbench for one goal, one barrier, one next step, and an honest review.">
<title>Dream to Action — One goal. One barrier. One next step.</title>
<style>__CSS__</style>
</head>
<body>
<a class="skip" href="#main">Skip to the workbench</a>
<header>
<p class="kicker">Dream to Action / Local prototype 0.1</p>
<h1>A fair chance starts<br>with a usable next step.</h1>
<p class="lead">Choose the life you want to work toward. Name what stands in the way. Separate your effort from the support and changes others need to provide.</p>
<div class="notice"><p><strong>You are not a score.</strong> This tool records a plan, not your worth. You can change your goal, ask for support, pause, or stop.</p></div>
<p class="hint no-print"><strong>No account or automatic upload.</strong> Entries stay in this page's memory until you export them. Exported files are unencrypted. Your browser or device may retain traces; this is not a secure vault. Closing or reloading can lose unsaved work.</p>
<noscript><div class="notice"><p>JavaScript is disabled. The form remains readable, but snapshots, exports, and review records require JavaScript. The companion PILOT.md is a plain-text alternative.</p></div></noscript>
</header>
<main id="main" tabindex="-1">
<p id="status" role="status" aria-live="polite" aria-atomic="true"></p>
<div id="errors" class="error" tabindex="-1" role="alert" aria-atomic="true" hidden></div>
<div class="layout">
<div>
<section class="panel" id="edit-panel" aria-labelledby="edit-title">
<h2 id="edit-title">Build one practical step</h2>
<p class="hint" id="form-help">Four fields are required: goal, barrier, next action, and evidence of progress. Everything else can remain unknown. Do not enter passwords, account numbers, or other sensitive identifiers.</p>
<form id="plan-form" novalidate autocomplete="off" aria-describedby="form-help">
<fieldset><legend>1. Your aim and the real barrier</legend>
<div class="field"><label for="goal">What life improvement are you working toward? (required)</label><textarea id="goal" name="goal" maxlength="4000" required aria-describedby="goal-help"></textarea><p class="hint" id="goal-help">Use your own words. A peaceful ordinary life counts; wealth is not the required goal.</p></div>
<div class="field"><label for="barrier">What is getting in the way? (required)</label><textarea id="barrier" name="barrier" maxlength="4000" required aria-describedby="barrier-help"></textarea><p class="hint" id="barrier-help">A barrier can be cost, access, time, missing information, a process, or something else. It is not a judgment about you.</p></div>
</fieldset>
<fieldset><legend>2. A step you choose</legend>
<div class="field"><label for="action">What is the next concrete action? (required)</label><textarea id="action" name="action" maxlength="4000" required aria-describedby="action-help"></textarea><p class="hint" id="action-help">Make it small enough to try. Asking for a necessary change can be the action.</p></div>
<div class="field"><label for="evidence">What observable change would count as progress? (required)</label><textarea id="evidence" name="evidence" maxlength="4000" required aria-describedby="evidence-help"></textarea><p class="hint" id="evidence-help">Measure whether the barrier changed or the action became usable, not how many boxes you checked.</p></div>
<div class="field"><label for="readiness">Is this step feasible and acceptable to you now?</label><select id="readiness" name="readiness"><option value="unknown">I do not know yet</option><option value="ready">I believe it is ready to try</option><option value="blocked">Support or a change is needed first</option><option value="paused">I choose to pause</option></select></div>
</fieldset>
<details id="support-details"><summary>Support, responsibility, and review (optional)</summary>
<fieldset><legend>3. Do not carry the whole burden yourself</legend>
<div class="field"><label for="owner">Who will take the next action?</label><input id="owner" name="owner" maxlength="4000" value="Me" aria-describedby="owner-help"><p class="hint" id="owner-help">Naming another person does not mean they have agreed. Record only a role or nickname when sufficient.</p></div>
<div class="field"><label for="support">What support or external change does the next action need?</label><textarea id="support" name="support" maxlength="4000" aria-describedby="support-help"></textarea><p class="hint" id="support-help">For example: an accessible form, transport, time, equipment, an introduction, or an institutional decision.</p></div>
<div class="field"><label for="helper">Who could provide that support or change?</label><input id="helper" name="helper" maxlength="4000" aria-describedby="helper-help"><p class="hint" id="helper-help">This is a proposed dependency, not an assigned obligation or a message to that person.</p></div>
<div class="field"><label for="supportStatus">Has the required support been confirmed?</label><select id="supportStatus" name="supportStatus"><option value="unknown">Unknown / not confirmed</option><option value="requested">Requested, awaiting an answer</option><option value="confirmed">Confirmed, according to my information</option><option value="unavailable">Not available</option><option value="not_needed">No outside support needed for this step</option></select></div>
<div class="field"><label for="constraints">What limits must the step respect?</label><textarea id="constraints" name="constraints" maxlength="4000" aria-describedby="constraints-help"></textarea><p class="hint" id="constraints-help">Time, cost, accessibility, consent, privacy, effects on others, or a reason to stop. No financial details are required.</p></div>
<div class="field"><label for="reviewDate">When will you review it? (optional date)</label><input id="reviewDate" name="reviewDate" type="date" aria-describedby="review-help"><p class="hint" id="review-help">A planning date only. This tool does not schedule or send reminders.</p></div>
<div class="field"><label for="kind">What kind of record is this?</label><select id="kind" name="kind"><option value="self_reported">My plan — self-reported</option><option value="illustrative">Illustrative example — not a real result</option></select></div>
</fieldset></details>
<div class="buttons"><button type="submit">Save a plan snapshot</button><button type="button" id="example" class="secondary">Load an example</button></div>
<p class="hint">Snapshots are kept in this open page, not automatically saved to disk. Each revision is added to the journal. Export the journal to keep it.</p>
</form>
</section>
<section class="panel" id="review-panel" aria-labelledby="review-title">
<h2 id="review-title">Record what actually happened</h2>
<p class="hint" id="review-form-help">Save a plan snapshot first. Record observations without turning a setback into a judgment about the person. Both fields below are required.</p>
<form id="review-form" novalidate autocomplete="off" aria-describedby="review-form-help">
<div class="field"><label for="outcome">What changed? (required)</label><select id="outcome" name="outcome"><option value="unresolved">Still unresolved / not enough evidence</option><option value="improved">The barrier was reduced or access improved</option><option value="achieved">The chosen goal was reached</option><option value="no_change">No improvement observed</option><option value="harm">Harm or an unacceptable cost was observed</option><option value="changed_goal">I chose a different goal or direction</option></select></div>
<div class="field"><label for="observation">What did you observe, and what remains unresolved? (required)</label><textarea id="observation" name="observation" maxlength="4000" required aria-describedby="observation-help"></textarea><p class="hint" id="observation-help">Distinguish what happened from what you think caused it. Do not include private information about other people.</p></div>
<button type="submit" id="record-review" class="secondary">Add an observation</button>
</form>
</section>
</div>
<div>
<section class="panel" id="preview-panel" aria-labelledby="preview-title">
<h2 id="preview-title" tabindex="-1">Your current plan snapshot</h2>
<p id="empty-preview">No snapshot yet. Start with the four required fields, or load the illustrative example.</p>
<div id="preview-content" hidden></div>
</section>
<section class="panel" id="file-panel" aria-labelledby="file-title">
<h2 id="file-title">Keep it under your control</h2>
<p class="hint">The journal preserves the current draft, snapshots, and observations. A readable report includes the same information. Neither file is encrypted.</p>
<div class="buttons"><button type="button" id="export-json">Export journal (.json)</button><button type="button" id="export-text" class="secondary">Export readable report (.txt)</button><button type="button" id="print" class="secondary">Print saved plan and history</button></div>
<p class="hint">Printing excludes unsaved draft edits. Exporting the journal or readable report includes them.</p>
<div class="field"><label for="import-file">Restore an exported journal (.json; at most 1 MiB)</label><input type="file" id="import-file" accept=".json,application/json" aria-describedby="import-help"><p class="hint" id="import-help">Import replaces the open journal after confirmation. A file is treated as unverified user data, never as executable instructions.</p></div>
<button type="button" id="clear" class="danger">Clear this open journal</button>
<p class="hint">Clearing does not delete files you already exported or remove browser/device traces.</p>
</section>
<section class="panel" id="history-panel" aria-labelledby="history-title">
<h2 id="history-title">Plan and observation history</h2>
<p class="hint">Entries remain separate in this interface. Timestamps come from the device or imported file; this is not a tamper-proof audit log or independently verified evidence.</p>
<p id="empty-history">No recorded entries.</p>
...
<footer>
<p class="rule">A plan is not an opportunity until the needed conditions are actually available.</p>
<p>This prototype helps organize action. It cannot supply housing, jobs, care, funding, legal rights, or another person's agreement. It does not decide eligibility, rank people, or make decisions for you.</p>
<p class="hint">Designed with native labeled controls, keyboard operation, and announced feedback. Automated checks do not replace testing with the people and assistive technologies who will use it. English-language prototype; not universal-access or WCAG certification.</p>
</footer>
...
csp = f"default-src 'none'; script-src 'sha256-{digest(JS)}'; style-src 'sha256-{digest(CSS)}'; connect-src 'none'; img-src 'none'; font-src 'none'; base-uri 'none'; form-action 'none'; object-src 'none'"
(ROOT/'app.js').write_text(JS,encoding='utf-8')
(ROOT/'style.css').write_text(CSS,encoding='utf-8')
page = HTML.replace('__CSP__',csp).replace('__CSS__',CSS).replace('__JS__',JS)
(ROOT/'dream-to-action.html').write_text(page,encoding='utf-8')
print('Built',ROOT/'dream-to-action.html',len(page.encode()),'bytes')
